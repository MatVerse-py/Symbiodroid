...
- CRUD de casos por usuário (isolamento por owner_id)
...
- Geração de dossiê Markdown com cadeia de custódia + timeline + flags + ledger + análise LLM
...
## Backlog
- P1: Suporte a PDF (pdfplumber/PyPDF2)
- P1: Diarização de áudio (separar vozes em chamadas)
- P2: Export PDF do dossiê (weasyprint)
- P2: Streaming SSE em `/process` para casos grandes
- P2: OpenAPI exportável para Custom GPT Actions (já gerado em /docs)
- P2: Webhook de conclusão (notificar UI externa quando processamento terminar)
- P3: Background processing (Celery/RQ) para casos com muitos áudios
